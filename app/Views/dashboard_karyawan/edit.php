    ...
    <div class="w-full">
        ...
        <section class="container mx-auto p-6 font-mono">
            ...
            <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg mt-4 bg-white p-6">
                <h1 class="text-2xl font-bold mb-4">Edit Pekerjaan</h1>
                <?= form_open('/dashboard_karyawan/update/' . $pekerjaan['id_pekerjaan'], ['class' => 'mb-4']); ?>
                <div class="mb-4">
                    ...
                    <select class="js-example-basic-single w-full" name="id_order">
                        <?php foreach ($list_order as $order) { ?>
                            <option <?= ($pekerjaan['id_order'] == $order['id_order']) ? 'selected' : '' ?> value="<?= $order['id_order'] ?>"><?= $order['nama_barang'] ?> : Rp. <?= $order['harga'] ?></option>
                        <?php } ?>
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select class="js-example-basic-single w-full" name="status">
                        <?php foreach ($statusOptions as $value => $label) { ?>
                            <option value="<?= $value ?>" <?= ($pekerjaan['status'] == $value) ? 'selected' : '' ?>>
                                <?= $label ?>
                            </option>
                        <?php } ?>
                    </select>
